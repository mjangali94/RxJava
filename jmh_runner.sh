...
while IFS= read -r benchmark; do
  ...
  java -jar "$JMH_JAR" $JMH_OPTIONS "$BENCHMARK_WITH_DOLLAR" >> "$RESULTS_FILE" 2>> "$ERROR_LOG"
...
echo "All benchmarks completed. Results saved to $RESULTS_FILE. Errors logged in $ERROR_LOG."