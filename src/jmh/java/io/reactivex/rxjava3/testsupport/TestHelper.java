...
public enum TestHelper {
    ...
    public static void assertCompositeExceptions(TestSubscriberEx<?> ts, Class<? extends Throwable>... classes) {
        ts.assertSubscribed().assertError(CompositeException.class).assertNotComplete();
    ...
    public static void assertCompositeExceptions(TestObserverEx<?> to, Class<? extends Throwable>... classes) {
        to.assertSubscribed().assertError(CompositeException.class).assertNotComplete();
     ...
     */
    public static <T> void checkBadSourceObservable(Function<Observable<T>, Object> mapper, final boolean error, final T goodValue, final T badValue, final Object... expected) {
        List<Throwable> errors = trackPluginErrors();
        try {
            ...
            if (o instanceof ObservableSource) {
                ...
                if (error) {
                    to.assertError(TestException.class).assertErrorMessage("error").assertNotComplete();
                } else {
            ...
            if (o instanceof Publisher) {
                ...
                if (error) {
                    ts.assertError(TestException.class).assertErrorMessage("error").assertNotComplete();
                } else {
     ...
     * @param expected the expected resulting values, null to ignore values received
     */
    public static <T> void checkBadSourceFlowable(Function<Flowable<T>, Object> mapper, final boolean error, final T goodValue, final T badValue, final Object... expected) {
        List<Throwable> errors = trackPluginErrors();
        try {
    ...
    public static final class TestConsumerExConverters<T> implements ObservableConverter<T, TestObserverEx<T>>, SingleConverter<T, TestObserverEx<T>>, MaybeConverter<T, TestObserverEx<T>>, CompletableConverter<TestObserverEx<Void>>, FlowableConverter<T, TestSubscriberEx<T>> {
    ...
    public static <T> void checkUndeliverableUponCancel(FlowableConverter<Integer, T> transform) {
        ...
        try {
            ...
            T result = Flowable.just(1).map(new Function<Integer, Integer>() {
    ...
    public static <T> void checkUndeliverableUponCancel(ObservableConverter<Integer, T> transform) {
        ...
        try {
            final SerialDisposable disposable = new SerialDisposable();
            T result = Observable.just(1).map(new Function<Integer, Integer>() {
