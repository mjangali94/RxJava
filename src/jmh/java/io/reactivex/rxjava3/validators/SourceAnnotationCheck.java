...
public class SourceAnnotationCheck {
        ...
        for (int j = 0; j < lines.size(); j++) {
            ...
            for (String typeName : TYPES_FORBIDDEN_NONNULL_TYPEARG) {
                ...
                if (line.matches(patternRegex)) {
                    errorCount++;
                    errors.append("L").append(j).append(" : @NonNull type argument should be on the arg declaration ").append(typeName).append("\r\n").append(" at ").append(fullClassName).append(".method(").append(f.getName()).append(":").append(j + 1).append(")\r\n");
                }
            }
            for (String typeName : TYPES_REQUIRING_NONNULL_TYPEARG_ON_FUNC) {
                if (line.matches(".*Function[\\d]?\\<.*, (\\? (extends|super) )?" + typeName + ".*")) {
                    errorCount++;
                    errors.append("L").append(j).append(" : Missing @NonNull type argument annotation on Function argument ").append(typeName).append("\r\n").append(" at ").append(fullClassName).append(".method(").append(f.getName()).append(":").append(j + 1).append(")\r\n");
                }
            }
        }
        if (errorCount != 0) {
    ...
    static final List<String> TYPES_REQUIRING_NONNULL_TYPEARG = Arrays.asList("Iterable", "Stream", "Publisher", "Processor", "Subscriber", "Optional");

    static final List<String> TYPES_FORBIDDEN_NONNULL_TYPEARG = Arrays.asList("Iterable", "Stream", "Publisher", "Processor", "Subscriber", "Optional");

    static final List<String> TYPES_REQUIRING_NONNULL_TYPEARG_ON_FUNC = Arrays.asList("Iterable", "Stream", "Publisher", "Processor", "Subscriber", "Optional", "Observer", "SingleObserver", "MaybeObserver", "CompletableObserver");
