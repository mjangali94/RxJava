...
public final class OperatorMatrixGenerator {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (Class<?> clazz : CLASSES) {
            ...
            for (Method m : clazz.getMethods()) {
                ...
                if (!name.equals("bufferSize") && m.getDeclaringClass() == clazz && !m.isSynthetic()) {
    ...
    static {
        ...
        for (String s : NOTES) {
            ...
            for (char c : classes) {
                NOTES_MAP.computeIfAbsent(method, v -> new HashMap<>()).put(String.valueOf(c), note);
            }
        }
    }
}
