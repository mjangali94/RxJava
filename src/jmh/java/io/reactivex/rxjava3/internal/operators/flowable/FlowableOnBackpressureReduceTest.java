...
public class FlowableOnBackpressureReduceTest extends RxJavaTest {
    ...
    public void simpleError() {
        TestSubscriberEx<Integer> ts = new TestSubscriberEx<>();
        Flowable.range(1, 5).concatWith(Flowable.error(new TestException())).onBackpressureReduce(TEST_INT_REDUCER).subscribe(ts);
        ts.assertTerminated();
    ...
    public void asynchronousDrop() {
        ...
        int m = 100000;
        Flowable.range(1, m).subscribeOn(Schedulers.computation()).onBackpressureReduce((previous, current) -> {
            // in that case it works like onBackpressureLatest
            // the output sequence of number must be increasing
            return current;
        }).observeOn(Schedulers.io()).subscribe(ts);
    ...
    public void asynchronousDrop2() {
        ...
        int m = 100000;
        Flowable.rangeLong(1, m).subscribeOn(Schedulers.computation()).onBackpressureReduce(Long::sum).observeOn(Schedulers.io()).subscribe(ts);
        ts.awaitDone(2, TimeUnit.SECONDS);
        ts.assertTerminated();
        assertValuesDropped(ts, m);
        long sum = 0;
