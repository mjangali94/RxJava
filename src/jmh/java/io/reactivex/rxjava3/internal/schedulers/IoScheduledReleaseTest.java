...
public class IoScheduledReleaseTest extends RxJavaTest {
    ...
    public void scheduledRelease() {
        ...
        try {
            Flowable.just("item").observeOn(Schedulers.io()).firstOrError().map(item -> {
                for (int i = 0; i < 50; i++) {
                    Completable.complete().observeOn(Schedulers.io()).blockingAwait();
                }
                return "Done";
            }).ignoreElement().test().awaitDone(5, TimeUnit.SECONDS).assertComplete();
        } finally {
